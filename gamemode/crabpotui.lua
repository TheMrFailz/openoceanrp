...
function crabpotinfo(box)
	local w = 400
	local h = 300
	local Frame = vgui.Create( "DFrame" )
	Frame:SetPos( ScrW() / 2 - w / 2, ScrH() / 2 - h / 2 )
	Frame:SetSize( w, h )
	Frame:SetTitle( "Crab Pot" )
	...
	local transboxb = vgui.Create("DButton", Frame)
	transboxb:SetText("Transfer to box")
	transboxb:SetPos(0,60)
	transboxb:SetSize(w/2,40)
	transboxb.DoClick = function()
		Frame:Remove()
		
		net.Start("transfishpot")
	...
	local deploybutton = vgui.Create("DButton", Frame)
	deploybutton:SetText("Deploy Pot")
	deploybutton:SetPos(w/2,60)
	deploybutton:SetSize(w/2,40)
	...
	net.Start("CPRequestCrabTable")
		net.WriteEntity(box)
	net.SendToServer()

	net.Receive("CPSendCrabTable", function(len, ply)
	...
	f2panel:SetPos( 0, 100)
	f2panel:SetSize( w, h - 100)
	...
	function potfishtable(fishtabledata, potcontents)
		for i=1, table.Count(fishtabledata) do
			...
			fishmodel:SetCamPos(campos - Vector(-fishtabledata[i][7],0,-10))
...
-- "open me!"
net.Receive("CPOpenGUI", function(len, ply)
